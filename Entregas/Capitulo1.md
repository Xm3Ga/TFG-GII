...
En este Trabajo de Fin de Grado se propone diseñar e implementar un conjunto de flujos de seguridad para agentes de inteligencia artificial, integrados en una plataforma B2B de gobernanza de agentes agnóstica de proveedor, que se encuentra en desarrollo en la startup Theia Craft.

La necesidad de implementar estos flujos de seguridad nacen de una observación real identificada en los entornos empresariales actuales. Estas organizaciones están desplegando cada vez más agentes de IA en sus operaciones, normalmente usando un conjunto de proveedores como Amazon Web Services, Microsoft Azure, Google Cloud u OpenAI. Gestionar la seguridad de todos estos agentes de manera centralizada es ahora mismo un problema que no tiene aún una buena solución establecida.
...
- **Fragmentación de la seguridad**: cada proveedor ofrece sus propias herramientas de seguridad, que por diseño, solo sirven para su plataforma. No existe una forma unificada de aplicar políticas de seguridad a agentes de varios proveedores.
- **Ausencia de controles de privilegios**: los agentes de IA suelen desplegarse con permisos excesivos, sin un análisis de los privilegios mínimos que realmente necesitan para operar.
...
- **Complejidad técnica**: las soluciones de seguridad existentes requieren conocimientos avanzados para su aplicación, lo que dificulta su adopción por equipos no especializados.

## Propuesta de solución

Para abordar este problema, este trabajo propone el diseño y desarrollo de misiones de seguridad dentro de la plataforma de gobernanza de agentes de IA Horizon de Theia Craft. Las misiones son flujos conversacionales guiados mediante un agente de IA, mediante los cuales el usuario interactúa con el agente de la plataforma para configurar y aplicar políticas de seguridad sobre sus agentes.

La solución se estructura en torno a tres conjuntos de misiones: creación y aplicación de guardarraíles, cumplimiento del principio de mínimo privilegio, y definición de niveles de autonomía con supervisión humana. Dado a la naturaleza evolutiva de este proyecto, es posible que el alcance vaya siendo modificado a posteriori, disminuyendo o aumentando el alcance mientras sea posible.
...
El presente TFG se desarrolla en el contexto de una startup tecnológica que está construyendo una plataforma B2B (Business-to-Business) para el manejo y la gobernanza de agentes de inteligencia artificial. El objetivo de la plataforma es desarrollar un punto único de control, agnóstico de proveedor, que permite a las empresas gestionar todos sus agentes de IA desde una misma interfaz, independientemente de si están desplegados en Amazon Bedrock, Azure OpenAI Service, Google Cloud Vertex AI u otros proveedores.

Su funcionamiento se basa en un sistema de misiones, que son flujos guiados por un modelo de IA en los que el cliente dialoga con el agente de la plataforma, y este le orienta paso a paso a través del proceso que corresponda.

La primera misión que todo cliente debe completar para empezar a usar la plataforma es la de conexión: el agente de la plataforma guía al usuario para crear cuentas de servicio o proveer credenciales, de forma que la plataforma pueda integrarse con su infraestructura y descubrir sus agentes desplegados. A partir de ahí se desbloquean misiones de diversa índole: seguimiento de costes, versionado, trazabilidad y, en particular, las misiones de seguridad que constituyen el objeto de este TFG.

En el momento de inicio de este proyecto, la plataforma se encuentra en fase pre-MVP (Minimum Viable Product). Las misiones de seguridad que se proponen formarán parte del primer conjunto funcional del producto. Dado a la naturaleza cambiante del proyecto, es posible que tanto el alcance como las misiones a describir cambien, aunque se ajustarán siempre al tiempo disponible y la viabilidad del producto.

### Solución existente aproximada

En el ámbito de la seguridad para agentes de IA, la solución más próxima al enfoque de este trabajo es Amazon Bedrock Guardrails (Amazon Web Services [AWS], 2024). Este es el mecanismo oficial que provee Amazon para sus propios agentes de IA. Este permite definir políticas de filtrado de contenido, bloqueo de temas sensibles, detección de información de identificación personal (PII) y protección frente a inyecciones de prompts.

El interés de esta solución reside en que permite aplicar políticas de seguridad configurables con bastante facilidad. El administrador puede definir qué temas están permitidos, qué tipo de información no debe aparecer en las respuestas y qué patrones de entrada deben ser bloqueados. Esto es una avance considerable respecto a la seguridad de agentes previa, que se basaba en la descripción de un buen system prompt.

Sin embargo, esta solución presenta grandes limitaciones para el problema que se aborda en este TFG. En primer lugar, funciona exclusivamente dentro del ecosistema de Amazon Bedrock; no es posible aplicar estas políticas sobre agentes desplegados en Azure, Google Cloud u otros proveedores. En segundo lugar, no aborda aspectos como la gestión de privilegios de los agentes ni la definición de niveles de autonomía con supervisión humana. Y en tercer lugar, su configuración se realiza mediante consola técnica, sin un flujo conversacional guiado que facilite la adopción por parte de equipos con distintos niveles de competencia técnica.

Por lo tanto, los Guardrails de Amazon serán un fuerte punto de partida e inspiración para el proyecto, pero se necesita más cobertura y más facilidad de implementación.

## Marco teórico

### Seguridad en sistemas de inteligencia artificial

La rápida transformación digital a nivel empresarial que ha potenciado la inteligencia artificial, ha generado un numeroso conjunto de riesgos de seguridad que difiere mucho de los riesgos tradicionales en ingeniería de software. Mientras que toda la ciberseguridad anterior había evolucionado conforme la infraestructura moderna se iba desarrollando, la rápida expansión de los modelos de IA y los agentes ha mostrado todo un continente nuevo de peligros y vulnerabilidades (OWASP, 2025).
...
- **LLM06 — Agencia excesiva**: concesión de demasiada autonomía a los agentes de IA para realizar diferentes tareas, especialmente tareas críticas en una organización, como procesamiento de pagos o acceso a bases de datos (OWASP, 2025).

### Guardarraíles para agentes de IA

El concepto de guardarraíles en inteligencia artificial hace referencia a los mecanismos de control que restringen el comportamiento de un modelo o agente dentro de unos límites predefinidos. Como dato importante, no todas las plataformas llaman a sus sistemas de seguridad igual. En este caso, guardaraíl representa correctamente lo que quiero explicar, así que usaré este término como expliación del concepto, aunque no todas las plataformas lo ofrezcan con el mismo nombre. Estas pueden operar a distintos niveles:
...
Los principales proveedores de servicios de IA en la nube han desarrollado sus propias medidas de seguridad. Además de Amazon Bedrock Guardrails, ya mencionado, Azure AI Content Safety (Microsoft, 2024) ofrece moderación de contenido mediante modelos especializados en la detección de contenido dañino y discurso de odio, limitado al ecosistema de Azure. Google Cloud Vertex AI (Google Cloud, 2024) incorpora filtros de seguridad configurables para sus modelos generativos, limitado a Google Cloud Platform.

En cuanto a código abierto, NVIDIA NeMo Guardrails (NVIDIA, 2024) permite añadir guardarraíles configurables a aplicaciones basadas en modelos de lenguaje mediante un lenguaje llamado Colang. Su enfoque es flexible pero requiere conocimientos técnicos avanzados. Guardrails AI (Guardrails AI, 2024) permite definir validadores para entradas y salidas de modelos, enfocados en la validación a nivel de aplicación individual.

### Principio de mínimo privilegio en sistemas de IA

El principio de mínimo privilegio es un concepto fundamental en seguridad informática que establece que una entidad (usuario, proceso o, en este caso, agente de IA) solo debe disponer de los permisos estrictamente necesarios para realizar su función (Saltzer & Schroeder, 1975). En el contexto que nos compete, esto es especialmente relevante, dado que muchos de estos agentes tienen acceso a muchas funciones de alto riesgo en las organizaciones.
...
La aplicación de este principio a agentes de IA requiere un análisis de las tareas que cada agente ejecuta, identificar los permisos mínimos necesarios y eliminar el exceso de privilegios. Este proceso, cuando se realiza manualmente sobre decenas o cientos de agentes, resulta inviable.

### Autonomía y supervisión humana

La autonomía de un agente de IA es el nivel de acciones que este puede tomar sin necesidad de intervención humana. El concepto de human-in-the-loop (HITL) explica la necesidad de incluir supervisión humana en los procesos automatizados, especialmente cuando las acciones del agente conllevan riesgos significativos (Shneiderman, 2022).

No todas las tareas tienen el mismo nivel de riesgo. Una búsqueda por internet tiene un impacto bajo si el agente comete un error, mientras que un procesamiento de pago, una modificación de datos personales o una decisión que afecta a la seguridad de un sistema requieren un nivel de supervisión mayor. La definición de diferentes niveles de autonomía por agente permiten que agentes con bajo riesgo trabajen de manera autónoma, mientras que las operaciones de alto riesgo necesitan aprobación humana.
...
Un agente de IA puede definirse como una entidad software que percibe información de su entorno, la procesa, toma decisiones y ejecuta acciones orientadas a un objetivo (Russell & Norvig, 2021). A diferencia de los modelos “normales” de IA, un agente implementa modelos de lenguaje con instrucciones personalizadas, herramientas, memoria, y diferentes mecanismos de control para aumentar la autonomía de los modelos tradicionales para abordar tareas más complejas.

La diferencia entre IA generativa e IA agéntica es relevante para este trabajo. La IA generativa tradicional responde a una entrada mediante la producción de una salida, normalmente textual (Wang et al., 2024). La IA agéntica incorpora el uso de herramientas, pasos intermedios y capacidad de verificación o control del flujo. Esta diferencia es importante porque las misiones de seguridad que se van a desarrollar no se limitan a generar recomendaciones de texto: requieren que el agente de la plataforma analice configuraciones reales de los agentes del cliente, interactúe con APIs de distintos proveedores y aplique cambios efectivos sobre la infraestructura.
...
**Calypso AI** (Calypso AI, 2024) ofrece pruebas y evaluación de seguridad para modelos de IA, incluyendo pruebas de penetración y evaluación del cumplimiento normativo. Se posiciona como herramienta de testing y auditoría, complementaria pero no sustitutiva de los mecanismos de protección en tiempo de ejecución.

### Marcos regulatorios y estándares

El panorama regulatorio en torno a la inteligencia artificial ha evolucionado significativamente, ofreciendo un marco normativo que las organizaciones deben cumplir:
...
Dado el analisis que he realizado, podemos ver que ninguna de las plataformas cumplen nuestras necesidades al 100%.

Las soluciones de guardarraíles de los proveedores cloud (Amazon Bedrock Guardrails, Azure AI Content Safety, Google Vertex AI) son las más maduras en cuanto a funcionalidad, pero presentan una limitación fundamental: solo operan dentro de sus respectivos ecosistemas. La mayoría de las organizaciones utilizan diferentes proveedores para sus agentes, por lo que es imposible aplicar las mismas políticas de seguridad de manera unificada.

Las plataformas de seguridad especializadas (Lakera Guard, Protect AI, Arthur AI, Calypso AI) aportan capacidades avanzadas, pero cada una se centra en un aspecto concreto de la seguridad. Ninguna de estas soluciones aborda todos los problemas planteados en este trabajo.

Las herramientas de código abierto (NeMo Guardrails, Guardrails AI) ofrecen flexibilidad, pero requieren conocimientos técnicos avanzados para su configuración y no proporcionan una experiencia guiada que facilite la adopción. Además, tampoco cubre el problema al completo.
...
En la tabla se puede ver que ninguna solución ofrece una solución total al problema planteado. Además, la característica de ser agnóstico de proveedor, que es fundamental para el contexto empresarial descrito, solo se consigue de forma parcial con las herramientas de código abierto o las plataformas de seguridad independientes.

Por todo ello, se justifica la necesidad de desarrollar una solución específica que, integrada en una plataforma de gobernanza agnóstica de proveedor, permita abordar la seguridad de los agentes de IA de forma centralizada y guiada.

## 2.2. Solución propuesta

Debido a todo lo expuesto en los apartados anteriores, la solución propuesta consiste en diseñar y desarrollar un conjunto de misiones de seguridad dentro de la plataforma Horizon.

La solución se estructura en torno a tres conjuntos de misiones:

**Misiones de guardarraíles**: el cliente inicia la misión y dialoga con el agente de la plataforma para configurar los diferentes guardaraíles para aplicar a sus agentes de IA. Incluyen sub-misiones para definir el tono y estilo de respuesta, bloquear temas prohibidos, activar la protección frente a inyecciones de prompts y activar la detección y anonimización de PII.

**Misión de cumplimiento del mínimo privilegio**: una misión que analiza los permisos asignados a cada agente de IA del cliente, calcula dinámicamente el conjunto mínimo de privilegios necesarios para sus tareas y propone la eliminación del exceso de privilegios.
...
Esta metodología es perfecta para este proyecto, ya que dada la naturaleza de fase temprana de la aplicación, es posible que tanto los requisitos como el alcance vayan cambiando con el tiempo.
